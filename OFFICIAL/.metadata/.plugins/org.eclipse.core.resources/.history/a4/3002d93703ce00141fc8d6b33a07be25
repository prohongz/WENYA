...
public class SimCanvas extends Canvas implements Runnable, Constants {
	...
	static final int VEH_COLORCODE = 1; // 0: own colors; 1: speed colorcode; 2:
										// VLA code
	...
	public SimCanvas(int choice_Szen, double density, double qIn, double perTr,
			double p_factor, double deltaB, double p_factorRamp,
			double deltaBRamp, int tsleep_ms) {

		newValues(choice_Szen, density, qIn, perTr, 33, p_factor, deltaB,
				tsleep_ms);
		...
		microstreet = new MicroStreet(2 * Math.PI * RADIUS_M, density,
				p_factor, deltaB, fcnr, choice_Szen);
	...
	public void setScales() {
		...
		if (false) {
			System.out.println("SimCanvas.setScales:" + " xsize_m=" + xsize_m
					+ " xsize=" + xsize + " ysize=" + ysize + ", l_straight_m="
					+ l_straight_m + ", rdlen_m=" + rdlen_m);
		}
		...
		textFontSmall = new Font("SansSerif", Font.PLAIN,
				(int) (0.8 * textHeight + 0.5));
		...
		diaHeight = (int) (0.5 * r2 - 0.5 * padding); // do not know why not
														// 1*padding
		diaWidth = (isRingroad) ? xsize - xc - r2 - 2 * padding
		...
		if (isRingroad) {
			...
			System.out.println("diaHeight=" + diaHeight + "diaWidth="
					+ diaWidth);
		}
		...
		while (Thread.currentThread() == runner) {
			...
			synchronized (microstreetLock) {
				...
				if ((time > T_GREEN) && (choice_Szen == 5)
					...
					traffLightTurnedGreen = true; // (only traffic light
													// scenario)
				}
				microstreet.update(time, TIMESTEP_S, choice_Szen, density, qIn,
			...
			// tsleepFrame_ms = tsleep_ms- simTime_ms - prepTime_ms -
			// avgPaintTime_ms;
			...
			if (true) {
				System.out.println("tsleep=" + tsleep_ms + ", tsleepFr="
						...
						+ prepTime_ms + ", avgPaintT=" + avgPaintTime_ms
						+ ", totPaintT=" + totalPaintTime_ms + ", totT="
						+ totalTimeThisStep_ms);
			}
		} // while (Thread.currentThread()== runner)
	} // public void run
	...
	public void start(int choice_Szen, double density) {
		...
		if (choice_Szen == 2) {
			double mergingpos = Math.PI * RADIUS_M + l_straight_m + 0.5
					* L_RAMP_M;
			System.out.println("SimCanvas.start: mergingpos=" + mergingpos
					+ ",l_straight_m=" + l_straight_m + ",total_rmplen_m="
					+ total_rmplen_m);
			onramp = new OnRamp(microstreet, total_rmplen_m, L_RAMP_M,
					mergingpos, p_factorRamp, deltaBRamp);
		}
	...
	private void paintSim(Graphics g) {
		...
		synchronized (microstreetLock) {
			...
			if (VEH_COLORCODE == 1) {
				if (choice_Geom == 0) {
					paintColormapSpeed(g, ((double) (xc)) / xsize + 0.05, 0.75,
							0.2, 0.10);
				} else {
			...
			if (((choice_Szen == 3) || (choice_Szen == 4) || (choice_Szen == 5))
				...
				imposeSpeedLimit(); // !!! Bring outside of drawing routine; not
									// confuse
	...
	private void updateCircle(Graphics g) {
		...
		for (int i = imax; i >= 0; i--) {
			...
			// martin
			Color vehColor = (showInsetdiagrams && (nr[i] == fcnr)) ? Color.blue
					: (Color) (colors.elementAt(i));
			String vehType = (lengthVeh[i] < 8) ? "car" : "truck";
			VehicleImage2D vehimg = new VehicleImage2D(lengthVeh[i],
					VEH_WIDTH_M, vehType, false, vehColor, scale);
	...
	private void updateU(Graphics g) {
		...
		for (int i = imax; i >= 0; i--) {
			PolyVehU pvu = new PolyVehU(pos[i], lane[i], lengthVeh[i],
					VEH_WIDTH_M, r1, r2, xc, l_straight, yc, (int) (rdlen));
		...
		if (choice_Szen == 2) {
			...
			for (int i = imaxOn; i >= 0; i--) {
				...
				VehicleImage2D vehimg = new VehicleImage2D(lengthVehOn[i],
						VEH_WIDTH_M, vehType, false, vehColor, scale);
	...
	private void prepareVehData() {
		...
		Vector lanes = microstreet.realLanes; // here (drawing) lanes always
												// real-valued!
		...
		for (int i = imax; i >= 0; i--) {
			...
			if (old_nr != nr[i]) { // overtaking during last step -> changed pos
				old_i[i] = (i + 1 <= imax) ? (i + 1) : 0; // old vehicle number
															// at i+1?
				...
				if (old_nr != nr[i]) { // veh index not at i+1 -> must be at i-1
					...
					old_nr = ((Integer) old_numbers.elementAt(old_i[i]))
							.intValue();
				}
			}
			...
			if (old_nr != nr[i]) { // noy yet ordered?
				...
				if (old_nr != nr[i]) {
					System.out
							.println("prepareVehData, 2nd ordering round,2!!");
					for (int j = -2; ((j >= -5) && (old_nr != nr[i])); j--) {
				...
				if (old_nr != nr[i]) {
					System.out
							.println("Error! old vehicle number not in neighbourhood!");
				}
			}

			old_pos[i] = ((Double) old_positions.elementAt(old_i[i]))
					.doubleValue();
		}
		...
		if (choice_Szen == 2) {
			for (int i = imaxOn; i >= 0; i--) {
				posOn[i] = ((Double) onramp.positions.elementAt(i))
						.doubleValue();
				lengthVehOn[i] = ((Double) onramp.lengths.elementAt(i))
						.doubleValue();
			}
		}
	...
	private void drawLines(Graphics g) {
		...
		if (choice_Geom == 0) {
			for (int i = 0; i < numberOfLines; i++) {
				...
				PolyVehCircle roadLine = new PolyVehCircle(pos_m, -1,
						lineLength, lineWidth, r1, r2, xc, yc);
		...
		if (choice_Geom == 1) {
			for (int i = 0; i < numberOfLines; i++) {
				double pos_m = i * (LINELENGTH_M + GAPLENGTH_M);
				PolyVehU roadLine = new PolyVehU(pos_m, -1, lineLength,
						lineWidth, r1, r2, xc, l_straight, yc, (int) (rdlen));
				g.fillPolygon(roadLine.xPoints, roadLine.yPoints, 4);
			}
		}
	}
	...
	public void newValues(int choice_Szen, double density, double qIn,
		...
		rdlen_m = (choice_Geom == 0) ? 2 * Math.PI * RADIUS_M : Math.PI
				* RADIUS_M + 2 * l_straight_m;

		System.out.println("SimCanvas.newValues():" + " choice_Szen= "
				+ choice_Szen + " choice_Geom= " + choice_Geom);

	} // newValues

	public void newValues2(double qIn, double perTr, double p_factor,
			double deltaB, double qRamp, double p_factorRamp,
			double deltaBRamp, int tsleep_ms) {
		...
		choice_Geom = 1;
		rdlen_m = Math.PI * RADIUS_M + 2 * l_straight_m;
		System.out.println("SimCanvas.newValues2():" + " choice_Szen= "
				+ choice_Szen + " choice_Geom= " + choice_Geom);
	} // newValues2
	...
	private void paintLegend(Graphics g) {
		...
		int xoff = (withProbeCar) ? metricsText.stringWidth(probeString)
				+ pkwLen : maxLenCarTruck + pkwLen;
		if (choice_Szen == 6) {
			xoff = metricsText.stringWidth(idString) + pkwLen;
		}

		int x0 = (int) (xRelRight * xsize) - xoff - lkwLen; // xsize,ysize in
															// pixels
		...
		if (withProbeCar) {
			g.setColor(Color.blue);
			g.fillRect(x0 + xoff, y0 + 2 * lineHeight - vehWidth, pkwLen,
					vehWidth);
		}
	}
	...
	private void paintTimes(Graphics g) {
		...
		if (choice_Szen == 5) {
			if (time < T_GREEN) {
				...
				g.setColor(Color.white);
				g.fillRect(xLeft + width_strCountdown, yTop, width_strTime,
						textHeight);
				g.setColor(Color.black);
				g.drawString(countdString, xLeft,
						(int) (yTop + 0.9 * textHeight));
			}
		}
	}
	...
	private void paintSymbols(Graphics g) {
		...
		if (choice_Szen == 5) {
			...
			g.fillRect(detx - radiusTL, dety - heightTL, 2 * radiusTL,
					4 * radiusTL);
		...
		if (v0_kmh >= V0_LIMIT_MAX_KMH) { // no speed limit
			...
			for (int i = -2; i <= 2; i++) {
				g.drawLine(xc - (int) (0.7 * radiusSS) + i * space, yc
						+ (int) (0.7 * radiusSS) + i * space, xc
						+ (int) (0.7 * radiusSS) + i * space, yc
						- (int) (0.7 * radiusSS) + i * space);
			}
		}
	...
	class PolyVehCircle {
		...
		public PolyVehCircle(double pos, double lane, double length,
				double width, int r1, int r2, int xc, int yc) {
			...
			double r = (lane >= 0) ? r1 + (0.25 + 0.5 * lane) * (r2 - r1) : r1
					+ 0.5 * (r2 - r1);
	...
	private void setInsetDiagrams() {
		...
		if (choice_Szen == 1) {
			...
			fcdxdv = new StatBufDiag(-10.0, 10.0, 0.0, 100.0, diaWidth,
					diaHeight, 400, Color.white, Color.black, diagram1,
					"Velocity Difference", "Headway", "", "Blue Car", 5.0,
					"5 m/s", 50.0, "50 m");
			...
			fcvt = new DynBufDiag(0.0, 60.0, -5.0, 5.0, diaWidth, diaHeight,
					...
					colors, "", "Acceleration+Velocity", 30.0, "-30s", ytics,
					yticLabels);
			Qrho = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight,
					2000, Color.white, Color.black, diagram3, "rho", "Q",
					"Detectors D1-D4", "Data from", 0.075, "75 Veh/km", 0.417,
					"1500 Veh/h");
			QrhoS = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight,
					1000, Color.white, Color.black, diagram4, "rho", "Q",
					"Detectors D1-D4", "Actual Data", 0.075, "75 Veh/km",
					0.417, "1500 Veh/h");
		...
		if (choice_Szen == 5) {
			...
			fcvt = new DynBufDiag(0.0, 120.0, -5.0, 5.0, diaWidth, diaHeight,
					120.0, Color.white, Color.black, diagram1, "time", ylabels,
					colors, "Blue Car", "Acceleration+Velocity", 60.0, "-60s",
					ytics, yticLabels);
			String[] ylabels1 = { "Q" };
			...
			Qt = new DynBufDiag(0.0, 60.0, 0.0, 0.8, diaWidth, diaHeight, 60.0,
					Color.white, Color.black, diagram2, "time", ylabels1,
					colors1, "", "Total Flow at D1", 30.0, "-30s", ytics1,
					yticLabels1);
		}
		...
		if ((choice_Szen == 2) || (choice_Szen == 3) || (choice_Szen == 4)) {
			...
			Qrho = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight,
					500, Color.white, Color.black, diagram1, "rho", "Q",
					"Detector at D2", "Measured Data", 0.075, "75 Veh/km",
					0.417, "1500 Veh/h");
			...
			if (false) {
				...
				Qt = new DynBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight,
						...
						ylabels1, colors1, "", "Total Flow at D1", 30.0,
						"-30s", ytics1, yticLabels1);
			}

			QrhoS = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight,
					100, Color.white, Color.black, diagram2, "rho", "Q",
					"Detector at D2", "Actual Data", 0.075, "75 Veh/km", 0.417,
					"1500 Veh/h");
			Qrho1 = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight,
					500, Color.white, Color.black, diagram3, "rho", "Q",
					"Detector at D1", "Measured Data", 0.075, "75 Veh/km",
					0.417, "1500 Veh/h");
			QrhoS1 = new StatBufDiag(0.0, 0.15, 0.0, 0.8, diaWidth, diaHeight,
					100, Color.white, Color.black, diagram4, "rho", "Q",
					"Detector at D1", "Actual Data", 0.075, "75 Veh/km", 0.417,
					"1500 Veh/h");
	...
	private void paintInsetDiagrams(Graphics g) {
		...
		if (choice_Szen == 1) {
			...
			g.drawString("D1", detx - (int) (0.5 * xoff), dety + distance
					+ yoff);
			...
			g.drawString("D4", detx - distance - xoff, dety
					+ (int) (0.5 * yoff));
		...
		if ((choice_Szen == 2) || (choice_Szen == 3) || (choice_Szen == 4)) {
			...
			double detpos = (choice_Szen == 4) ? 0.4 * CIRCUMF_M
					: 0.6 * CIRCUMF_M;
	...
	/**
	 * draw the speed color code if colorCode==1 if condition is at the calling
	 * location, calls final method VehicleImage2D.colormapSpeed(...) xrel=0..1
	 * from left, yrel=0..1 from bottom
	 */

	private void paintColormapSpeed(Graphics g, double xrelCenter,
			double yrelCenter, double widthRel, double heightRel) {
