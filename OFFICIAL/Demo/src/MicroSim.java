...
public class MicroSim extends Applet implements Constants {
	...
	private double qIn = (CHOICE_SCEN_INIT == 2) ? Q_INIT2 / 3600.
			: (CHOICE_SCEN_INIT == 3) ? Q_INIT3 / 3600.
					: (CHOICE_SCEN_INIT == 4) ? Q_INIT4 / 3600.
							: Q_INIT5 / 3600.;
	...
	// when changing SIM_BG_COLOR, also change it in
	// de/trafficsimulation/SimCanvas.java
	...
	private Scrollbar sb_IDMv0 = AdjustmentMethods.getSB(V0_MIN_KMH,
			V0_MAX_KMH, V0_INIT_KMH, false);
	...
	private SimCanvas simCanvas; // =new SimCanvas (2, density, qIn, perTr,
									// p_factor, deltaB, p_factorRamp,
									// deltaBRamp, tsleep_ms);// simulation
									// field
	...
	public void init() {
		...
		// set default scenario: Default CHOICE_SCEN_INIT; overridden by cmd
		// line in html
		...
		simCanvas = new SimCanvas(choice_Scen, density, qIn, perTr, p_factor,
				deltaB, p_factorRamp, deltaBRamp, tsleep_ms);// simulation field
		...
		int i_warpfactor = (int) (1000 * TIMESTEP_S / tsleep_ms); // !! at
																	// initial
		...
		pRamp.add(label_qIn2 = new Label(String.valueOf(flow_invh)
				+ str_vehperh), gbconstr);
		pSource.add(label_qIn3 = new Label(String.valueOf(flow_invh)
				+ str_vehperh), gbconstr);
		...
		pClosedSystem.add(
				label_perTr1 = new Label(String
						.valueOf((int) (100 * FRAC_TRUCK_INIT_CIRCLE)) + " %"),
				gbconstr);
		pRamp.add(label_qRamp = new Label(String.valueOf(flowRamp_invh)
				+ str_vehperh), gbconstr);
		pSource.add(
				label_perTr35 = new Label(String.valueOf(truckPerc) + " %"),
				gbconstr);
		pLanechange.add(label_deltaB6 = new Label(String.valueOf(deltaB)
				+ " m/s^2"), gbconstr);
		...
		pRamp.add(label_perTr2 = new Label(String.valueOf(truckPerc) + " %"),
				gbconstr);
		pSource.add(
				label_v0_limit = new Label(String
						.valueOf((int) (3.6 * v0_limit)) + " km/h"), gbconstr);
		pLanechange.add(label_simSpeed6 = new Label(str_warpfactor
				+ str_framerate), gbconstr);
		...
		pSource.add(
				label_simSpeed3 = new Label(str_warpfactor + str_framerate),
				gbconstr);
		...
		AdjustmentMethods.addScrollbar(pIDMparams, 1, sb_IDMv0, str_IDMv0,
				label_IDMv0);
		AdjustmentMethods.addScrollbar(pIDMparams, 2, sb_IDMa, str_IDMa,
				label_IDMa);
		AdjustmentMethods.addScrollbar(pIDMparams, 3, sb_IDMb, str_IDMb,
				label_IDMb);
		AdjustmentMethods.addScrollbar(pIDMparams, 4, sb_IDMT, str_IDMT,
				label_IDMT);
		AdjustmentMethods.addScrollbar(pIDMparams, 5, sb_IDMs0, str_IDMs0,
				label_IDMs0);
	...
	private void makeGlobalLayout() {
		...
		System.out.println("MicroSim.makeGlobalLayout:" + "Applet-size="
				...
				+ ", Client-size=" + usedWidth + " X " + usedHeight);
		...
		int sbLeft = isCircular ? (int) (0.5 * (simSize - sbWidth)) // 0.5 =
																	// center
		...
		pButtons.setBounds(getInsets().left, buttWinTop, usedWidth,
				buttWinHeight);
		this.add(pButtons);
		...
		simCanvas.setBounds(getInsets().left, simWinTop, usedWidth,
				simWinHeight);
		this.add(simCanvas);
	}
	...
	private final static boolean sbBroken = (new Scrollbar(
			Scrollbar.HORIZONTAL, 20, 10, 0, 20).getValue() != 20);
	...
	public boolean handleEvent(Event evt) {
		...
		case Event.SCROLL_ABSOLUTE:
			int i_warpfactor = (int) (1000 * TIMESTEP_S / tsleep_ms); // !!Veraend
																		// Sb
			String str_warpfactor = String.valueOf(i_warpfactor)
					+ "."
					+ String.valueOf((int) (10000 * TIMESTEP_S / tsleep_ms - 10 * i_warpfactor));
			...
			if (evt.target == sb_simSpeed1) {
				...
				if (dens_invkm != 1000 * density) {
					...
					label_density.setText(String.valueOf(dens_invkm)
							+ str_vehperkm);
				}
	...
	public boolean action(Event event, Object arg) {
		...
		int i_warpfactor = (int) (1000 * TIMESTEP_S / tsleep_ms); // 1000 new
																	// scen
		String str_warpfactor = String.valueOf(i_warpfactor)
				+ "."
				+ String.valueOf((int) (10000 * TIMESTEP_S / tsleep_ms - 10 * i_warpfactor));
		...
		if (event.target == button1) {
			...
			label_density.setText(String.valueOf(DENS_INIT_INVKM)
					+ str_vehperkm);
		...
		if (event.target == button2) {
			...
			p_factorRamp = 0.;
			deltaBRamp = DELTABRAMP_INIT; // negative shift threshold for
											// onramp!
		...
		if (event.target == button3) {
			...
			label_v0_limit.setText(String.valueOf((int) (v0_limit * 3.6))
					+ " km/h");
