...
public class MicroStreet implements Constants {
	...
	protected Random random = new Random(42); // for truck perc. and veh.
												// numbers
	...
	public MicroStreet(double length, double density, double p_factor,
		...
		if (choice_Szen == 1) {
			...
			for (int i = 0; i < nCars; i++) {
				int lane = (i % 2 == 0) ? 1 : 0;
				street.insertElementAt(new Car(roadLength - (i + 1) * distance,
						vNew, lane, idmCar, polite, PKW_LENGTH_M, colorCar, i),
						i);
		...
		if (choice_Szen == 6) {
			...
			for (int i = 0; i < nCars; i++) {
				...
				if (i % 7 == 0) {
					if (i != 49)
						street.insertElementAt(new Obstacle(roadLength
								- (i + 1) * 20.0, lane, 5., 9999), index);
					if (i == 49)
						street.insertElementAt(new Obstacle(roadLength + 30.0
								- (i + 1) * 20.0, lane, 5., 9999), index);
				} else {
					if (i < ((int) (0.5 * nCars))) {
						street.insertElementAt(new Car(roadLength - (i + 1)
								* 20.0, 0.0, lane, idmCar, inconsiderate,
		...
		if (choice_Szen == 5) {
			...
			street.addElement(new Car(5.0, idmCar.Veq(roadLength), 0, idmCar,
					polite, PKW_LENGTH_M, colorCar, 2));
		}
		...
		if (choice_Szen == 3) {
			...
			for (int i = 0; i < n_obst; i++) {
				...
				street.insertElementAt(new Obstacle(pos, 0,
						obst_elem_length + 1, i), i);
			}
			street.insertElementAt(new Car(5.0, idmTruck.Veq(roadLength), 0,
					idmTruck, polite, LKW_LENGTH_M, colorTruck, n_obst), n_obst);
		}
		...
		if (choice_Szen == 4) {
			...
			street.insertElementAt(new Car(5.0, sync1Car.Veq(roadLength), 0,
					sync1Car, polite, PKW_LENGTH_M, colorCar, 0), 0);
		}
	...
	public void applyLocalPerturbation() {
		...
		// the first and last two cars are BCCcars on each lane during
		// acceleration()
		...
		System.out.println("MicroStreet.applyLocalPert at veh " + i + " imax="
				+ imax);
	}
	...
	protected Vector setDistances() { // neglect gaps in front of first/last veh
		...
		int iFrontCarsBoth = (iFirstLeft > iFirstRight) ? iFirstLeft
				: iFirstRight;
		...
		for (int i = iFrontCarsBoth + 1; i < imax; i++) {
			...
			double distance = ((Double) positions.elementAt(iFront))
					.doubleValue()
	...
	protected void adaptToNewDensity(double density, double perTr) {
		...
		if (nCars_wished > nCars) {
			System.out.println("nCars_wished=" + nCars_wished + " nCars="
					+ nCars);
	...
	/*
	 * <Treiber aug06> Methode veraendert, so dass auch bei 0 Fz auf beiden
	 * Spuren neue Fz eingefuehrt werden koennen (Ringstrasse) </Treiber>
	 */
	...
	private void insertOneVehicle(double perTr) {
		...
		System.out.println("MicroStreet.insertOneVehicle: maxgap="
				+ (int) maxgap + " index=" + i_maxgap + " pos=" + pos_maxgap
				+ " lane=" + lane_maxgap);
		...
		if (maxgap > mingap) {
			...
			street.insertElementAt((new Car(posNew, vNew, lane_maxgap,
					modelNew, changemodelNew, lNew, colorNew, randInt)),
					i_maxgap);
		}
	...
	protected void insertBCCars(int choice_BC) {
		...
		if (choice_BC == 0) { // periodic BC
			...
			street.insertElementAt(new BCCar(upLeftPos + roadLength, upLeftVel,
					0, idmCar, PKW_LENGTH_M), 0);
			street.insertElementAt(new BCCar(upRightPos + roadLength,
					upRightVel, 1, idmCar, PKW_LENGTH_M), 0);
			int imax = street.size();
			street.insertElementAt(new BCCar(downLeftPos - roadLength,
					downLeftVel, 0, idmCar, PKW_LENGTH_M), imax);
			imax++;
			street.insertElementAt(new BCCar(downRightPos - roadLength,
					downRightVel, 1, idmCar, PKW_LENGTH_M), imax);
		}
	...
	protected void accelerate(double dt) {
		...
		// are virtual BCCcars such that lanecHange(), accelerate() are always
		// defined
	...
	protected void ioFlow(double dt, double qIn, double perTr, int choice_BC) {
		...
		if (choice_BC == 0) {
			...
			if (imax >= 0) { // at least one vehicle present
				...
				while (temp_car.position() > roadLength) {
					...
					// and insert it at the end with position reduced by
					// roadLength
		...
		if (choice_BC == 1) {
			...
			if (imax >= 0) {
				...
				while ((imax >= 0) && // (imax>=0) first to prevent
										// indexOutOf...!
			...
			if (nin > 1.0) { // new vehicle imposed by the inflow BC
				...
				if ((i_lu >= 0) && (i_ru >= 0)) {
					imax = street.size() - 1;
					int laneLastVeh = ((Moveable) (street.elementAt(imax)))
							.lane();
					int lane = (laneLastVeh == 0) ? 1 : 0; // insert on other
															// lane
					int iPrev = lastIndexOnLane(lane); // index of previous
														// vehicle
					double space = ((Moveable) (street.elementAt(iPrev)))
							.position();
					double vPrev = ((Moveable) (street.elementAt(iPrev)))
							.velocity();

					// enough space for new vehicle to enter? (!red)

					if (!(red = (space < spaceMin))) {
						MicroModel carmodel = (choice_Szen != 4) ? idmCar
								: sync1Car;
						MicroModel truckmodel = (choice_Szen != 4) ? idmTruck
								: sync1Truck;
						double rand = random.nextDouble() * 1.0;
						int randInt = Math.abs(random.nextInt());
						MicroModel modelNew = (rand < perTr) ? truckmodel
								: carmodel;
						LaneChange changemodelNew = (rand < perTr) ? polite
								: inconsiderate;
						...
						double lNew = (rand < perTr) ? LKW_LENGTH_M
								: PKW_LENGTH_M;
						Color colorNew = (rand < perTr) ? colorTruck : colorCar;

						imax = street.size();
						street.insertElementAt(new Car(0.0, vNew, lane,
								modelNew, changemodelNew, lNew, colorNew,
								randInt), imax);
					}
				}
	...
	protected void sort() {
		...
		while (!sorted) {
			...
			for (int i = 1; i < imax; i++) {
				...
				double p_front = ((Moveable) street.elementAt(i - 1))
						.position();
				if (p_back > p_front) {
					sorted = false;
		...
		if (nr_max >= 0) {
			while ((i <= nr_max) && (!carFound)) {
				if (((Moveable) street.elementAt(i)).lane() == lane) {
					// if
					// (((((Moveable)street.elementAt(i)).lane())==lane)&&(flag==0)){
					carFound = true;
				}
				// System.out.println
